...
/* PMP Modification - Begin */
#ifdef DUMP_DEP
    gen_helper_dump_memory_read(tcg_const_tl(s->pc_start), a0, tcg_const_tl(1<<(idx&(~MO_SIGN))), t0);
#endif
/* PMP Modification - End */
...
/* PMP Modification - Begin */
#ifdef DUMP_DEP
    gen_helper_dump_memory_write(tcg_const_tl(s->pc_start), a0, tcg_const_tl(1<<(idx&(~MO_SIGN))), t0);
#endif
/* PMP Modification - End */
...
/* PMP Modification - Begin */
        gen_helper_branch(cpu_T0, tcg_const_tl(s->base.pc_next), tcg_const_tl(next_eip), tcg_const_tl(val), tcg_const_tl(next_eip));
...
/* PMP Modification - Begin */
        gen_helper_branch(cpu_T0, tcg_const_tl(s->base.pc_next), tcg_const_tl(val), tcg_const_tl(val), tcg_const_tl(next_eip));
...
/* PMP Modification - Begin */
#ifdef DUMP_INSN
    if (need_handle(pc_start)) {
        FILE *insn = fopen(buff_insn, "a+");
        fprintf(insn, TARGET_ABI_FMT_lx"\n", pc_start - program_code_offset);
        fclose(insn);  
    }
#endif
/* PMP Modification - End */
...
/* PMP Modification - Begin */
/*
            if (op == OP_SUBL && opreg == R_ESP && val >= 0) {
                //printf("initilize local variables: %lx\n", (unsigned long)(pc_start));

                g_hash_table_remove_all(cover);
                fill_arguments(val);
            }
*/
/* PMP Modification - End */
...
/* PMP Modification - Begin */
            gen_helper_indirect_call(cpu_T0, tcg_const_tl(s->base.pc_next), cpu_T0, tcg_const_tl(next_eip));
/* PMP Modification - End */
...
/* PMP Modification - Begin */
            gen_helper_indirect_jmp(cpu_T0, tcg_const_tl(s->base.pc_next), cpu_T0, tcg_const_tl(s->pc - s->cs_base));
/* PMP Modification - End */
...
/* PMP Modification - Begin */
/*
        if (reg == R_ESP && ((modrm & 7) | REX_B(s)) == R_EBP) {
...
/* PMP Modification - Begin */
            fill_arguments(0x1000);
            gen_helper_dump_function_call(cpu_T1, tcg_const_tl(s->base.pc_next - s->cs_base), tcg_const_tl(tval), tcg_const_tl(next_eip));
/* PMP Modification - End */
...
/* PMP Modification - Begin */
    if (need_handle(dc->base.pc_next)) {

        g_hash_table_replace(recover, (void*)(long)(dc->base.pc_next), (void*)(pc_next - dc->base.pc_next)); 

#ifdef DUMP_LOG
        FILE *log = fopen(buff_log, "a+");
        target_disas(log, cpu, dc->base.pc_next, pc_next - dc->base.pc_next);
        fclose(log);
#endif
...
void fill_memory(target_ulong start, size_t size) 
{
    for (size_t i = 0; i < size / TARGET_LONG_SIZE; i++) {
        target_ulong val = fill_value();
        memcpy((void*)(intptr_t)(start + i * TARGET_LONG_SIZE), &val, TARGET_LONG_SIZE);
    }
}

void fill_arguments(size_t size)
{
    for (size_t i = 1; i <= size / TARGET_LONG_SIZE; i++) {
        AddressParts a = {R_SS, R_ESP, -1, 0, -i * TARGET_LONG_SIZE};
        TCGv ea = gen_lea_modrm_1(a);
        tcg_gen_mov_tl(cpu_A0, ea);
        target_ulong val = fill_value();
        tcg_gen_movi_tl(cpu_T0, val);
        tcg_gen_qemu_st_tl(cpu_T0, cpu_A0, 0, (TARGET_LONG_SIZE == 8 ? MO_64 : MO_32));
    }
}

int need_handle(target_ulong insn_addr)
{
    int need = (insn_addr >= program_code_start && insn_addr <= program_code_end);
    return need;
}

int valid_addr(target_ulong insn_addr)
{
    int valid = (insn_addr > ALLOCATE_SIZE);
    return valid;
}

void dump_env(CPUX86State *env)
{
    printf("-------------------------------------\n");

#ifdef TARGET_X86_64
    printf("rip: "TARGET_ABI_FMT_lx"\n"
           "rbp: "TARGET_ABI_FMT_lx"\nrsp: "TARGET_ABI_FMT_lx"\nrsi: "TARGET_ABI_FMT_lx"\nrdi: "TARGET_ABI_FMT_lx"\n"
           "rax: "TARGET_ABI_FMT_lx"\nrbx: "TARGET_ABI_FMT_lx"\nrcx: "TARGET_ABI_FMT_lx"\nrdx: "TARGET_ABI_FMT_lx"\n", 
           env->eip, 
           env->regs[R_EBP], env->regs[R_ESP], env->regs[R_ESI], env->regs[R_EDI],
           env->regs[R_EAX], env->regs[R_EBX], env->regs[R_ECX], env->regs[R_EDX]);

    printf("r8: "TARGET_ABI_FMT_lx"\nr9: "TARGET_ABI_FMT_lx"\nr10: "TARGET_ABI_FMT_lx"\nr11: "TARGET_ABI_FMT_lx"\n"
           "r12: "TARGET_ABI_FMT_lx"\nr13: "TARGET_ABI_FMT_lx"\nr14: "TARGET_ABI_FMT_lx"\nr15: "TARGET_ABI_FMT_lx"\n",
           env->regs[8], env->regs[9], env->regs[10], env->regs[11],
           env->regs[12], env->regs[13], env->regs[14], env->regs[15]);
#else
    printf("eip: "TARGET_ABI_FMT_lx"\n"
           "ebp: "TARGET_ABI_FMT_lx"\nesp: "TARGET_ABI_FMT_lx"\nesi: "TARGET_ABI_FMT_lx"\nedi: "TARGET_ABI_FMT_lx"\n"
           "eax: "TARGET_ABI_FMT_lx"\nebx: "TARGET_ABI_FMT_lx"\necx: "TARGET_ABI_FMT_lx"\nedx: "TARGET_ABI_FMT_lx"\n", 
           env->eip, 
           env->regs[R_EBP], env->regs[R_ESP], env->regs[R_ESI], env->regs[R_EDI],
           env->regs[R_EAX], env->regs[R_EBX], env->regs[R_ECX], env->regs[R_EDX]);
#endif
    ...
    printf("-------------------------------------\n");
    //for (target_ulong pointer = env->regs[R_ESP]-0x40; pointer <= env->regs[R_EBP]; pointer+=sizeof(target_ulong))
      //  printf(TARGET_ABI_FMT_lx": "TARGET_ABI_FMT_lx"\n", pointer, *(target_ulong *)(intptr_t)(pointer));
    printf("-------------------------------------\n");
/*
    for (target_ulong pointer = 0x0; pointer <= 0x200; pointer+=sizeof(target_ulong)) 
        printf(TARGET_ABI_FMT_lx": "TARGET_ABI_FMT_lx"\n", pointer, *(target_ulong *)(intptr_t)(pointer));
    printf("-------------------------------------\n");

    for (target_ulong pointer = guest_base+0x0; pointer <= guest_base+0x200; pointer+=sizeof(target_ulong)) 
        printf(TARGET_ABI_FMT_lx": "TARGET_ABI_FMT_lx"\n", pointer, *(target_ulong *)(intptr_t)(pointer));
    printf("-------------------------------------\n");
*/
    target_disas(stdout, CPU(x86_env_get_cpu(env)), env->eip, 0x10);
}

void helper_test(target_ulong arg)
{
    printf("test: %lx\n", (unsigned long)(arg));
}

target_ulong helper_dump_function_call(target_ulong caller_site, target_ulong callee_entry, target_ulong next_addr)
{
    GHashTable *cover = g_hash_table_new(g_direct_hash, g_direct_equal);
    g_hash_table_replace(cover, (void*)(0), (void*)(next_addr)); 
    g_queue_push_tail(stack, cover);

#ifdef DUMP_CG
    if (!need_handle(caller_site) || !need_handle(callee_entry)) return;

    struct pair *item = malloc(sizeof(struct pair));
    item->first = caller_site;
    item->second = callee_entry;

    if (g_sequence_lookup(cgs, item, pair_compare, NULL)) { 
        free(item); 
        return;
    }
    g_sequence_insert_sorted(cgs, item, pair_compare, NULL);

    FILE *cg = fopen(buff_cg, "a+");
    fprintf(cg, TARGET_ABI_FMT_lx"->"TARGET_ABI_FMT_lx"\n", caller_site, callee_entry);
    fclose(cg);
#endif
...
void helper_dump_memory_read(target_ulong insn_addr, target_ulong mem_addr, target_ulong mem_size, target_ulong mem_val)
{
    if (!need_handle(insn_addr)) return;
#ifdef DUMP_MEM
    FILE *mem = fopen(buff_mem, "a+");
    fprintf(mem, "r "TARGET_ABI_FMT_lx" "TARGET_ABI_FMT_lx" "TARGET_ABI_FMT_lx" "TARGET_ABI_FMT_lx"\n", insn_addr - program_code_offset, mem_addr, mem_size, mem_val);
    fclose(mem);
#endif

    for (target_ulong addr = mem_addr; addr < mem_addr + mem_size; addr++) {
        target_ulong use = insn_addr;
        target_ulong define = (target_ulong)(long)(g_hash_table_lookup(memory, (void*)(long)(addr)));
        if (!define) continue;
        ...
        if (g_sequence_lookup(deps, item, pair_compare, NULL)) {
            free(item);
            continue;
        }
        g_sequence_insert_sorted(deps, item, pair_compare, NULL);

        FILE *dep = fopen(buff_dep, "a+");
        fprintf(dep, TARGET_ABI_FMT_lx"->"TARGET_ABI_FMT_lx"\n", use, define);
        fclose(dep);
    }
}

void helper_dump_memory_write(target_ulong insn_addr, target_ulong mem_addr, target_ulong mem_size, target_ulong mem_val)
{
    if (!need_handle(insn_addr)) return;

#ifdef DUMP_MEM
    FILE *mem = fopen(buff_mem, "a+");
    fprintf(mem, "w "TARGET_ABI_FMT_lx" "TARGET_ABI_FMT_lx" "TARGET_ABI_FMT_lx" "TARGET_ABI_FMT_lx"\n", insn_addr - program_code_offset, mem_addr, mem_size, mem_val);
    fclose(mem);
#endif

    for (target_ulong addr = mem_addr; addr < mem_addr + mem_size; addr++) {
        g_hash_table_replace(memory, (void*)(long)(addr), (void*)(long)(insn_addr));
    }
}

target_ulong helper_indirect_jmp(target_ulong insn_addr, target_ulong dest_addr, target_ulong next_addr)
{

    target_ulong target_addr = dest_addr;

    if (!need_handle(insn_addr)) return target_addr;

    GSequenceIter *iter = g_sequence_get_iter_at_pos(flip, flip_idx);
    struct pair *obj = (g_sequence_iter_is_end(iter) ? NULL: (struct pair *)(g_sequence_get(iter)));

    if (obj && obj->first == insn_addr) {
        target_addr = obj->second;
        flip_idx++;
    } else if (!valid_addr(target_addr)) {
        target_addr = next_addr;
    }

#ifdef DUMP_LOG
    FILE *log = fopen(buff_log, "a+");
    fprintf(log, "indirect jmp: "TARGET_ABI_FMT_lx"->"TARGET_ABI_FMT_lx" dest:"TARGET_ABI_FMT_lx" next:"TARGET_ABI_FMT_lx"\n", 
            insn_addr - program_code_offset, target_addr - program_code_offset, 
            dest_addr - program_code_offset, next_addr - program_code_offset);
    fclose(log);
#endif

#ifdef DUMP_TRACE
    FILE *trace = fopen(buff_trace, "a+");
    fprintf(trace, TARGET_ABI_FMT_lx"#"TARGET_ABI_FMT_lx"\n", insn_addr - program_code_offset, target_addr - program_code_offset);
    fclose(trace);
#endif
...
void helper_hook_return(target_ulong ret_addr)
{
    GHashTable *cover = g_queue_pop_tail(stack);
    if (g_hash_table_lookup(cover, (void*)(0)) == ret_addr) g_hash_table_destroy(cover);
    else g_queue_push_tail(stack, cover);
}


target_ulong helper_indirect_call(target_ulong insn_addr, target_ulong dest_addr, target_ulong next_addr)
{
    GHashTable *cover = g_hash_table_new(g_direct_hash, g_direct_equal);
    g_hash_table_replace(cover, (void*)(0), (void*)(next_addr)); 
    g_queue_push_tail(stack, cover);
...
//printf("indirect calll: "TARGET_ABI_FMT_lx", "TARGET_ABI_FMT_lx", "TARGET_ABI_FMT_lx","TARGET_ABI_FMT_lx"\n", insn_addr, dest_addr, target_addr, next_addr);
#ifdef DUMP_TRACE
    if (need_handle(insn_addr) && need_handle(target_addr)) {
        FILE *trace = fopen(buff_trace, "a+");
        fprintf(trace, TARGET_ABI_FMT_lx"$"TARGET_ABI_FMT_lx"\n", insn_addr - program_code_offset, target_addr - program_code_offset);
        fclose(trace);
    }
...
/*
    if (!valid_addr(target_addr)) target_addr = next_addr;

#ifdef DUMP_LOG
    FILE *log = fopen(buff_log, "a+");
    fprintf(log, "indirect call: "TARGET_ABI_FMT_lx"->"TARGET_ABI_FMT_lx" dest:"TARGET_ABI_FMT_lx" next:"TARGET_ABI_FMT_lx"\n", 
            insn_addr - program_code_offset, target_addr - program_code_offset, 
            dest_addr - program_code_offset, next_addr - program_code_offset);
    fclose(log);
#endif
...
target_ulong helper_branch(target_ulong insn_addr, target_ulong current_branch, target_ulong true_branch, target_ulong false_branch)
{
    target_ulong target_branch = current_branch;

    if (!need_handle(insn_addr)) return target_branch;

    GSequenceIter *iter = g_sequence_get_iter_at_pos(flip, flip_idx);
    struct pair *obj = (g_sequence_iter_is_end(iter) ? NULL: (struct pair *)(g_sequence_get(iter)));

    target_ulong black_target = (target_ulong)(g_hash_table_lookup(blackhole, (void*)(long)(insn_addr)));
    ...
    else if (black_target == 'T' || black_target == 'F') {
        if (black_target == 'T') return true_branch; 
        else if (black_target == 'F') return false_branch;
    }
    else {
        target_ulong loop_target = (target_ulong)(g_hash_table_lookup(loops, (void*)(long)(insn_addr)));
        if (loop_target == 'T' && current_branch == false_branch) {
            target_branch = true_branch;
        }
        else if (loop_target == 'F' && current_branch == true_branch) {
            target_branch = false_branch;
        }
    }

    GHashTable *cover = g_queue_peek_tail(stack);
 
    long count = (long)(g_hash_table_lookup(cover, (void*)(long)(target_branch))) + 1;
    g_hash_table_replace(cover, (void*)(long)(target_branch), (void*)count); 

#ifdef DUMP_LOG
    FILE *log = fopen(buff_log, "a+");
    if (current_branch != target_branch)
        fprintf(log, "flip branch: "TARGET_ABI_FMT_lx"->"TARGET_ABI_FMT_lx" true:"TARGET_ABI_FMT_lx" false:"TARGET_ABI_FMT_lx"\n", 
                insn_addr - program_code_offset, target_branch - program_code_offset, 
                true_branch - program_code_offset, false_branch - program_code_offset);
    fprintf(log, "LOOP: "TARGET_ABI_FMT_lx" -> "TARGET_ABI_FMT_lx" : %ld\n", insn_addr - program_code_offset, target_branch - program_code_offset, count);
    fclose(log);
#endif

    if (/*g_hash_table_lookup(branch, (void*)(unsigned long)(insn_addr)) && */count > LOOP_LIMIT) {
        target_branch = (target_branch == true_branch ? false_branch : true_branch);
    }

#ifdef DUMP_TRACE
    FILE *trace = fopen(buff_trace, "a+");
    fprintf(trace, TARGET_ABI_FMT_lx":%s\n", insn_addr - program_code_offset, (target_branch == true_branch ? "T" : "F"));
    fclose(trace);
#endif
